...
class Problem:
    def __init__(
        self, tag, deadline, languages, category="", name="", points=0, manual=False
    ):
        self.tag = tag
...
class AdjuleManager:
    ...
    def load_problems(self, problems_path):
        ...
        with open(problems_path) as f:
            for line in f:
                problem_tag, deadline, languages, category = line.rstrip("\n").split(
                    "\t"
                )
                deadline = dateparser.parse(
                    f"{deadline} 23:59:59.999999", ["%d.%m.%y %H:%M:%S.%f"]
                )
                languages = [l.replace(" ", "") for l in languages.split(",")]
                problem = Problem(problem_tag, deadline, languages, category)
        ...
        for problem in progressbar.progressbar(self.problems):
            ...
            while not problem.name:
                problem.name = self.driver.find_element_by_css_selector(
                    "div.small-6.cell.card-section > h1"
                ).text
                time.sleep(1)
    ...
    def log_in(self):
        ...
        password.send_keys(getpass.getpass(f"\nPassword for {self.login} in adjule:"))
            ...
            if next_button.get_attribute("class") == "disabled":
                ...
                next_button = WebDriverWait(self.driver, 10).until(
                    EC.element_to_be_clickable(
                        (By.CSS_SELECTOR, "li.pagination-next > button")
                    )
                )
                next_button.click()
    ...
    def get_student_data(self, student_url):
        ...
        profile_name_element = WebDriverWait(self.driver, 10).until(
            EC.presence_of_element_located(
                (By.CSS_SELECTOR, "ul.no-bullet a.profilename")
            )
        )
        ...
        if "'" in profile_name:
            ...
            name = "{} {}".format(
                profile_name_array[0].strip(), profile_name_array[2].strip()
            )
        ...
        while exist_submission:
            ...
            try:
                WebDriverWait(self.driver, 5).until(
                    EC.presence_of_element_located((By.CSS_SELECTOR, acc_sub_selector))
                )
    ...
    def add_problems_to_students(self):
        ...
        for i, student in enumerate(self.students):
            logger.info(
                "Evaluating student ({}/{}): name='{}', nick='{}', number='{}'.".format(
                    i + 1,
                    len(self.students),
                    student.name,
                    student.nick,
            ...
            for problem in progressbar.progressbar(self.problems):
                student.problems.append(self.evaluate_problem(student, problem))
    ...
    def update_marks(self):
        ...
        with open(self.marks_path, "w") as f:
            ...
            for problem in self.problems:
                ...
                print(
                    f"{problem.name} ({problem.tag}, {problem.category}) {languages}\t",
                    end="",
                    file=f,
                )
            categories = {p.category: 0 for p in self.problems}
            print("\t".join(categories.keys()), file=f)
            for student in self.students:
                ...
                for problem in student.problems:
                    categories[problem.category] += problem.points
                    if problem.manual:
                        print(f"{problem.points}*\t", end="", file=f)
                    else:
                        print(f"{problem.points}\t", end="", file=f)
                print("\t".join(map(str, categories.values())), file=f)
...
def get_args():
    parser = argparse.ArgumentParser(
        ...
        epilog="""example usage: ./adjule_points.py --login siulkilulki --tasks zadania.tsv --marks oceny-1ca.tsv --group ppr1ca2018
NOTE: Script will prompt for adjule password (on command-line) after reaching log in page.""",
    ...
    parser.add_argument(
        "--tasks",
        required=True,
        help="""Path to tsv file with following columns <problem_tag> <deadline> <allowed_languages>.
...
""",
    ...
    parser.add_argument(
        ...
        dest="headless",
        action="store_false",
        help="Runs real browser. Default: Runs browser in headless mode (faster).",
    )
...
def main():
    ...
    adjule_manager = AdjuleManager(
        driver, args.login, args.group, args.tasks, args.marks
    )
    adjule_manager.run()
    driver.quit()
